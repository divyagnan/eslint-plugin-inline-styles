            ...
            if (attr.value.expression.properties) {
              attr.value.expression.properties.map(prop => {
                ...
                if (
                   ...
                   */
                  !(
                    _.endsWith(prop.value.raw, "'") ||
                    _.endsWith(prop.value.raw, '"')
                  ) &&
                  /**
                   * Check if it ends with some unit value.
                   * Technically this isn't needed since you 
                   * can't have an identifier directly after
                   * a number.
                   */
                  !_.some(units, _.endsWith(prop.value.value))
                ) {
